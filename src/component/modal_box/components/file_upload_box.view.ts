...
import ItemBar from "../../bar/item_bar.view";
import { Icon } from "../../../icon/all_icon.view";

interface FileUploadBoxProp {
  show: boolean;
  onClose: () => void;
}

@View
class FileUploadBox implements FileUploadBoxProp {
  @Prop show = required;
  ...
  uniqueNameHolder = this.generateUniqueName(
    "file-folder",
    this.allFilterNames
  );
      ...
      div().class("px-4 pt-5");
      {
        div().class("sm:flex sm:items-start");
        {
          div().class("mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left");
          {
            ...
            div().class(
              " mt-2 min-h-40 max-h-72 overflow-y-auto overscroll-contain scroll-able modalbox-content"
            );
            {
              div().class("flex flex-wrap gap-1 pt-6 pb-12");
              {
                ItemBar(
                ...
                ItemBar("The Art of Double Bass Playing").dataTags([
                  "PDF",
                  "2.5MB",
                ]);
                ItemBar(
                ...
                ItemBar("DW28(Loops for Davis)").dataTags(["PDF", "12MB"]);
                ItemBar(
                  "DW31 Loops for Edgar Froese for widenen piano four hands and tape (2017)(1)"
                ).dataTags(["PDF", "8MB"]);
                ItemBar("DW 1.2").dataTags(["PDF", "6MB"]);
...
export default FileUploadBox as Pretty as Typed<FileUploadBoxProp>;
